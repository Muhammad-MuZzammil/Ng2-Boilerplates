...
var path = {
  ...
  libjs: [
    { dist: 'lib/shim', src: 'node_modules/core-js/client/shim.min.js' },    
    ...
    { dist: 'lib/systemjs.config', src: 'src/systemjs.config.js' },
  ],
  libcss: [],
  libfonts: [],
...
// Copy Global Css
gulp.task('copy:globalCss', function () {
  return gulp.src(path.indexCss)
    .pipe(gulp.dest(path.dist));
})
// Copy Html
...
// Build Project
gulp.task('build', sequence('clean', 'copy:index', 'copy:globalCss', 'copy:html', 'copy:css', 'copy:lib', 'copy:libjs', 'copy:libcss', 'copy:fonts', 'copyImages', 'transpile'));
...
// Watch Task
gulp.task('watch', ['watchts', 'watchcss', 'watchhtml', 'watchindex','watchGlobalCss']);
...
// watch global css
gulp.task('watchGlobalCss', function () {
  return watch(path.indexCss)
    .pipe(gulp.dest(path.dist));
})
